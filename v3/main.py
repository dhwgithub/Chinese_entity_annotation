...
def save_file():
    ...
    :return:
    '''
    global text_model, cur_file, pre_text_by_draw
    ...
    text_content.pop()
    tf = open(cur_file, 'r', encoding='utf-8')
    tf = tf.read().split("\n")
    texts = ""
    i = 0
    ...
    if not ok:
        f = open(setting.LOG_FILE_NAME, 'a', encoding='utf-8')
        f.write(save_path + " ==>>> 存在" + str(SUM) + "个/行标注不完全情况（详情请查看前述文件内容）\n")
        f.close()
    return True
...
def add_label(k):
    ...
    LEN = int(len(text_model.get(str(r0) + '.0', str(r0 + 1) + '.0')[:-1]))
    LEN = str(r0) + "." + str(LEN)
    text_model.config(state=tk.NORMAL)
...
if __name__ == '__main__':
    main()
    save_file()
    messagebox.showinfo("提示", "已自动为您保存~")
