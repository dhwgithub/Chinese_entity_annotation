...
def select_file_or_dir(types):
    ...
    if types == 'dir_file':
        ...
        # 获取所有文件
        files_list_of_cur_dir = natsorted(glob.glob(dir_of_files + '\*_seg.tsv'))
    ...
    # 选择文件
        ...
        if not cur_file.endswith('_seg.tsv'):
...
def save_file():
    ...
    # 文件保存目录
    save_res_file = os.path.join(dir_of_files, "result")
    if not os.path.exists(save_res_file):
        os.mkdir(save_res_file)
    res_file = os.path.join(save_res_file, str(cur_file).split("\\")[-1])
...
def open_file():
    ...
    try:
        ...
        origin_cur_file = cur_file.split("_seg.tsv")[0] + "_temp.tsv"
...
def modify_labels():
    ...
    :return:
    """
    global text_model, is_modified, cur_line_num, cur_line_end, origin_cur_file

    # print("选中区间：{}行{}列".format(cur_line_num, cur_line_end))
    ...
    if str(content).count("<*") > 0 or str(label).count("*>") > 0 or str(label).count("^") > 0:
        messagebox.showinfo("提示", "请正确选择标签~")
        return

    # 用户交互修改
    label_dict_ori = setting.labels_dict  # 原始字典
    label_dict_ori_revert = {v: k for k, v in label_dict_ori.items()}  # 原始字典翻转
    # print(label_dict_ori)
    # print(label_dict_ori_revert)
    label_dict_keys = list(setting.labels_dict.keys())  # 字典键列表
    try:
        init_label = [index for index, key in enumerate(label_dict_keys)
                      if key == label_dict_ori_revert.get(label)][0] + 1
        modify_data = simpledialog.askstring(title='修改标签',
                                             prompt=setting.modify_tag_info,
                                             initialvalue=init_label)
        if modify_data is None:  # 取消修改
            return
        ...
        if modify_data == "":  # 删除标签
            # print("删除标签\t 编辑行：{}".format(cur_line_num))  # 行号从1开始
            modify_cur_line = cur_line[:start] + str(content) + cur_line[end+2:]
            # print("修改行为：{}".format(modify_cur_line))
            ...
            modify_data = int(modify_data) - 1  # 用户输入标签key下标
            new_label = label_dict_ori[label_dict_keys[modify_data]]  # 新标签
            # print("修改标签为：{}".format(new_label))

            modify_cur_line = cur_line[:start + 2 + len(content)] + "^" + str(new_label) + cur_line[end:]
        ...
        messagebox.showinfo("提示", "请正确输入标签~")
        return
...
def key_board(event):
    ...
    if event.keysym == 'Right' or event.keysym == 'd' or event.keysym == 'D':  # 下一张
        ...
        show_want_file()
    elif event.keysym == "M" or event.keysym == "m":  # 修改标签
        modify_labels()
...
def add_label(k):
    ...
    if origin_cur_file is None:
        messagebox.showinfo("提示", "请选择文件文件")
        return
    ...
    for index in range(len(index_start)):
        ...
        # print(l, r, cur_line_start, cur_line_end)
        # 部分重叠及小嵌套判断
        if (l <= int(cur_line_start) <= r) or (l <= int(cur_line_end) - 1 <= r):
            messagebox.showinfo("提示", "请勿嵌套标签！！！")
            return
        # 包含其他标签判断
        if (int(cur_line_start) <= l <= int(cur_line_end) - 1) or \
                (int(cur_line_start) <= r <= int(cur_line_end) - 1):
            messagebox.showinfo("提示", "请勿嵌套标签！！！")
            return

    # 修该选中区间标签信息
    cur_label = '<*' + str(cur_label) + '^' + setting.labels_dict[k] + '*>'
    modify_cur_line = cur_line[: int(cur_line_start)] + cur_label + cur_line[int(cur_line_end):]

    # 在界面删除当前行信息并添加新内容
    text_model.config(state=tk.NORMAL)
    # print(str(cur_line_num) + ".0", str(cur_line_num) + "." + str(len(cur_line)))
    text_model.delete(str(cur_line_num) + ".0", str(cur_line_num) + "." + str(len(cur_line)))
    # print(modify_cur_line)
    text_model.insert(str(cur_line_num) + ".0", modify_cur_line)
    text_model.config(state=tk.DISABLED)
