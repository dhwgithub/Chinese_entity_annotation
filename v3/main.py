...
def open_file():
    ...
    global dir_of_files, pic_cur_file, add_w, add_h, GET_ROW
    GET_ROW[0] = 1
    try:
        pre_text_by_draw = []
...
def main():
    ...
    # frame_label_canvas.pack(side="left")
    frame_label.pack(side="left")
    frame_list.pack(side="left")

    # frame_label = tk.Frame(frame_label_canvas).place(width=180, height=180)
    # vbar = tk.Scrollbar(frame_label_canvas, orient=tk.VERTICAL)  # 竖直滚动条
    # vbar.place(x=180, width=20, height=180)
    # vbar.configure(command=frame_label_canvas.yview)
    # frame_label = tk.Frame(frame_label_canvas).pack(side="top")
    ...
    # # 滚动条
    # vbar = tk.Scrollbar(frame_label_canvas, orient=tk.VERTICAL)  # 竖直滚动条
    # vbar.place(x=0, width=20, height=180)
    # vbar.configure(command=frame_label_canvas.yview)
    # frame_label_canvas.create_window((90, 240), window=frame)
